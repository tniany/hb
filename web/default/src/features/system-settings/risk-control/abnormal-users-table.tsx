...
import { getAbnormalUsers, banUser } from './api'
import type { AbnormalUserStat } from './types'

type AbnormalUsersTableProps = {
  startTimestamp?: number
  endTimestamp?: number
}

export function AbnormalUsersTable({
  startTimestamp,
  endTimestamp,
}: AbnormalUsersTableProps) {
  ...
  const banMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setBanTarget(null)
      queryClient.invalidateQueries({ queryKey: ['abnormal-users'] })
    },
    onError: () => {
    ...
    <>
      <Card>
        <CardHeader className='flex flex-row items-center justify-between space-y-0'>
          <CardTitle>{t('Abnormal Users Detection')}</CardTitle>
