          ...
          <div className='grid grid-cols-1 gap-2.5 sm:gap-3 md:grid-cols-3'>
            {securityActions.map((item) => (
              <button
                ...
                onClick={item.action}
                className={`flex items-center gap-3 rounded-lg border p-3 text-left transition-colors md:flex-col md:gap-2 md:p-4 md:text-center ${
                  item.disabled
                    ? 'pointer-events-none cursor-not-allowed opacity-50'
                    : 'hover:bg-muted/50'
                } ${
                  item.variant === 'destructive'
                    ? `border-destructive/30 ${item.disabled ? '' : 'hover:border-destructive/50 hover:bg-destructive/5'}`
                    : ''
                }`}
              >
                <div
